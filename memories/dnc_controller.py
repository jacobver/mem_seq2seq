...
from neucom.utils import softmax, expand_dims
...
class Controller(nn.Module):
    def __init__(self, input_size=1, output_size=1, read_heads=1,
        ...
        self.mem_output_weights = nn.Parameter(
            torch.randn(self.mem_size * self.read_heads,
                        self.output_size).uniform_(-initrange, initrange)
        )

        self.register_parameter('memory output', self.mem_output_weights)
    ...
    def parse_interface_vector(self, interface_vector):
        ...
        parsed['read_keys'] = interface_vector[:,
                                               0:r_keys_end].contiguous().view(*r_keys_shape)
        parsed['read_strengths'] = interface_vector[:,
                                                    r_keys_end:r_strengths_end].contiguous().view(*r_strengths_shape)
        parsed['write_key'] = interface_vector[:,
                                               r_strengths_end:w_key_end].contiguous().view(*w_key_shape)
        parsed['write_strength'] = interface_vector[:,
                                                    w_key_end].contiguous().view(-1, 1)
        parsed['erase_vector'] = interface_vector[:, w_key_end +
                                                  1:erase_end].contiguous().view(*erase_shape)
        parsed['write_vector'] = interface_vector[:,
                                                  erase_end:write_end].contiguous().view(*write_shape)
        parsed['free_gates'] = interface_vector[:,
                                                write_end:free_end].contiguous().view(*free_shape)
        parsed['allocation_gate'] = expand_dims(
            interface_vector[:, free_end].contiguous(), 1)
        parsed['write_gate'] = expand_dims(
            interface_vector[:, free_end + 1].contiguous(), 1)
        parsed['read_modes'] = interface_vector[:,
                                                free_end + 2:].contiguous().view(*modes_shape)
    ...
    def process_input(self, X, last_read_vectors, rnn_state):
        """
    ...
    def final_output(self, pre_output, new_read_vectors):
        ...
        flat_read_vectors = new_read_vectors.view(
            -1, self.mem_size * self.read_heads)

        final_output = pre_output + \
            torch.mm(flat_read_vectors, self.mem_output_weights)
        # apply_dict(locals())
        return final_output
