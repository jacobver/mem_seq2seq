...
class Memory(nn.Module):
    ...
    def init_memory(self, batch_size):
        ...
        """
        mem_list = [
            Variable(torch.zeros(batch_size, self.mem_slot, self.mem_size).fill_(
                1e-6), requires_grad=True),  # initial memory matrix
            Variable(torch.zeros(batch_size, self.mem_slot),
                     requires_grad=True),  # initial memory usage vector
            Variable(torch.zeros(batch_size, self.mem_slot),
                     requires_grad=True),  # initial precedence vector
            Variable(torch.zeros(batch_size, self.mem_slot, self.mem_slot),
                     requires_grad=True),  # initial link matrix

            Variable(torch.zeros(batch_size, self.mem_slot).fill_(
                1e-6), requires_grad=True),  # initial write weighting
            Variable(torch.zeros(batch_size, self.mem_slot, self.read_heads).fill_(
                1e-6), requires_grad=True),  # initial read weighting
            Variable(torch.zeros(batch_size, self.mem_size,
                                 self.read_heads).fill_(1e-6), requires_grad=True)
        ]  # initial read vector
        if self.use_cuda:
    ...
    def update_usage_vector(self, usage_vector, read_weights, write_weight, free_gates):
        ...
        updated_usage = (usage_vector + write_weight -
                         usage_vector * write_weight) * retention_vector
        # apply_dict(locals())
    ...
    def update_link_matrix(self, precedence_vector, link_matrix, write_weight):
        ...
        updated_link_matrix = reset_factor * link_matrix + \
            torch.bmm(write_weight, precedence_vector)
        updated_link_matrix = (1 - self.I).expand_as(updated_link_matrix) * \
            updated_link_matrix  # eliminates self-links
    ...
    def update_read_weights(self, lookup_weights, forward_weight, backward_weight, read_mode):
        ...
        backward_mode = expand_dims(read_mode[:, 0, :].contiguous(), 1).expand_as(
            backward_weight) * backward_weight
        lookup_mode = expand_dims(read_mode[:, 1, :].contiguous(), 1).expand_as(
            lookup_weights) * lookup_weights
        forward_mode = expand_dims(read_mode[:, 2, :].contiguous(), 1).expand_as(
    ...
    def update_read_vectors(self, memory_matrix, read_weights):
        ...
        updated_read_vectors = torch.bmm(
            memory_matrix.transpose(1, 2), read_weights)
    ...
    def write(self, memory_matrix, usage_vector, read_weights, write_weight,
        ...
        new_memory_matrix = self.update_memory(
            memory_matrix, new_write_weight, write_vector, erase_vector)
        new_link_matrix = self.update_link_matrix(
            precedence_vector, link_matrix, new_write_weight)
        new_precedence_vector = self.update_precedence_vector(
            precedence_vector, new_write_weight)
    ...
    def read(self, memory_matrix, read_weights, keys, strengths, link_matrix, read_modes):
        ...
        lookup_weight = self.get_content_address(
            memory_matrix, keys, strengths)
        forward_weight, backward_weight = self.get_directional_weights(
            read_weights, link_matrix)
        new_read_weights = self.update_read_weights(
            lookup_weight, forward_weight, backward_weight, read_modes)
        new_read_vectors = self.update_read_vectors(
            memory_matrix, new_read_weights)
        # apply_dict(locals())
